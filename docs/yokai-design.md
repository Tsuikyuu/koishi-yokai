...
```text
硬触发：配置启用的 @ 当前机器人、回复其消息、角色名前缀、角色名任意包含，或待办事项到期
...
四个消息识别开关彼此独立；角色名前缀要求完整名称后为消息结束、空白、标点或符号。前缀和任意包含只读取当前 preset 的 `persona.name`，不读取插件名
或 Koishi 全局昵称。关闭某项硬回复不会抹去原始识别事实；它仍可作为指向性或社会相关度证据。硬触发可以绕过活跃度阈值，并使用
300～800 ms 的短 debounce 合并紧随其后的补充消息；普通
...
- 分享与群内近期话题高度相关的内容；
- 在合适空档加入正在进行的讨论；
- 在合适社交空档，基于人格兴趣、当前状态或已有自我记忆自由开启话题。

主动行为受频道冷却、每日预算和关系阈值限制。主动机会不要求角色一定发言，最终生成可以保持沉默。默认不主动私聊。
...
```text
direct        启用的 @、回复当前机器人消息，以及角色名前缀/包含
...
启用的直接 @ 当前机器人或回复其消息硬回复后，为当前用户和话题建立短期“讨论租约”。角色名前缀/包含
硬回复不建立租约。租约记录角色与群聊作用域、参与者、
锚点消息、开始时间、空闲到期时间、绝对期限和剩余回合。

租约规则：

- 用户第一次通过已启用的 @ 或回复自身规则触发时自动开启。
...
7. 内置 direct、activity、engagement、schedule 和 initiative 响应机制。
8. 持久化定时任务与短期讨论租约。
...
12. 少量由未完话题、高相关近期内容或低频角色内生动机触发的主动发言。
