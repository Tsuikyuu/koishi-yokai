...
`schedule.cancel`，均在发送角色消息前执行且结果不回灌模型。创建动作必须引用当前作用域内已存档的
来源消息，并携带去重键；同一作用域和去重键的消息重放只会得到同一任务，内容不一致的复用会被拒绝。

时间由宿主按 `schedule.timeZone` 解析，可使用带日期的 `YYYY-MM-DDTHH:mm[:ss]`，或表示下一次当地
时刻的 `HH:mm[:ss]`。调度器只等待数据库中最近的一项任务；重启后，错过不超过
`schedule.gracePeriodMs`（默认 `300000` ms）的任务立即提交 scheduled 唤醒，超过宽限则标记过期。
在同一 `instanceId` 仅有一个活跃 Yokai runtime 的前提下，每个 occurrence 在提交模型或发送前先通过
数据库条件更新和进程内串行门认领，因此该 runtime 内并发和重启恢复时最多触发一次；
重复任务会跳过停机期间已错过的周期并推进到严格晚于当前时间的下一次。scheduled 唤醒绕过普通
活跃度与频道冷却，只使用 `reserved` 调用预算，并按任务保存的 platform、selfId、guild 和 channel
选择精确机器人发送。`schedule.enabled=false` 会停止调度并隐藏这些能力，但保留已有数据库记录；
`schedule.contextLimit` 默认为 `8`、最大 `32`。
...
`instanceId` 同时是状态与调度的单写者边界；并行部署时，每个活跃 Yokai 实例必须使用不同的 ID。
